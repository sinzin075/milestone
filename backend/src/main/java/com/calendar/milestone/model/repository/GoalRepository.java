        ...
        Goal goal = jdbcTemplate.queryForObject(sql, new RowMapper<Goal>() {
            ...
            public Goal mapRow(ResultSet rs, int rowNum) throws SQLException {
                ...
                if (timestamp_created != null) {
                    goalMap.setCreatedAt(timestamp_created.toLocalDateTime());
                }
                if (timestamp_updated != null) {
                    goalMap.setUpdatedAt(timestamp_updated.toLocalDateTime());
                }
                if (timestamp_deleted != null) {
                    goalMap.setDeletedAt(timestamp_deleted.toLocalDateTime());
                }
                return goalMap;
            }
        }, id);
